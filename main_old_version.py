...
class AccountDialog(QDialog):
    def __init__(self, parent=None):
        ...
        # Tabela de contas
        self.table = QTableWidget(0, 5)  # 0 linhas, 4 colunas (ID, Login, Senha, Posição, Server)
        self.table.setHorizontalHeaderLabels(["ID", "Login", "Password", "Position", "Server"])
    ...
    def update_table(self):
        ...
        for i, account in enumerate(self.accounts):
            ...
            self.table.setItem(i, 1, QTableWidgetItem(account["login"]))
            self.table.setItem(i, 2, QTableWidgetItem("*" * len(account["password"])))
            self.table.setItem(i, 3, QTableWidgetItem(str(account["position"])))
            self.table.setItem(i, 4, QTableWidgetItem(str(account["server"])))
...
class AutoLoginWindow(QMainWindow):
    ...
    def perform_login(self, pids):
        ...
        # Garante que temos PIDs suficientes para as contas
        max_logins = min(len(pids), len(self.accounts))

        print(f"Iniciando login para {max_logins} contas em sequência:")
        for i in range(max_logins):
            print(f"  PID {pids[i]} -> Conta: {self.accounts[i]['login']}")
    ...
    def start_auto_login(self):
        ...
        try:
            ...
            # Inicia um cliente para cada conta cadastrada
            num_accounts = len(self.accounts)
            # print(f"Iniciando {num_accounts} clientes para as contas cadastradas")
            
            # Cria e inicia o thread do cliente
            self.client_thread = ClientThread(client_path, num_accounts, self.initial_pids)
